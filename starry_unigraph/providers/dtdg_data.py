...
from starry_unigraph.backends.flare import PartitionData, Route, TensorData
from starry_unigraph.core import SnapshotRoutePlan
...
def build_partition_input_from_raw_dataset(raw_dataset: dict[str, Any]) -> dict[str, Any]:
    dataset = list(raw_dataset.get("dataset", []))
    return {
        "num_nodes": int(raw_dataset["num_nodes"]),
        "num_edges": int(raw_dataset["num_edges"]),
        "num_snapshots": int(raw_dataset["num_snapshots"]),
        "merged_edge_count": int(sum(int(snapshot["edge_weight"].numel()) for snapshot in dataset)),
        "snapshots": dataset,
    }
...
def build_dtdg_partitions(
    ...
    snapshots = list(graph_data.get("snapshots", []))
    merged_edge_index = _merge_snapshot_edges(snapshots)
