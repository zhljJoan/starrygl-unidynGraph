...
from starry_unigraph.core import DTDGBatch, DTDGWindowState, SnapshotRoutePlan
from starry_unigraph.backends.flare import PartitionData, STGraphBlob, STGraphLoader
...
def _split_bounds(total: int, split: str, split_ratio: dict[str, float]) -> tuple[int, int]:
    ...
    raise KeyError(f"Unsupported DTDG split: {split}")
...
@dataclass
class ChunkedDTDGLoader:
    snaps: int
    window_size: int
    route_plan: SnapshotRoutePlan
    chunk_manifest: list[dict[str, Any]]
    split_ratio: dict[str, float] | None = None
    window_state: DTDGWindowState = field(init=False)
    cursor: int = 0

    def __post_init__(self) -> None:
        self.window_state = DTDGWindowState(window_size=self.window_size)
        self.split_ratio = _normalize_split_ratio(self.split_ratio)
    ...
    def _split_range(self, split: str) -> range:
        start, end = _split_bounds(total=max(0, self.snaps), split=split, split_ratio=self.split_ratio or {})
        return range(start, end)

    def iter_train(self, split: str = "train") -> Iterable[DTDGBatch]:
        for index in self._split_range(split):
            self.cursor = index + 1
            yield self.load_snapshot(index, split=split)
...
class FlareRuntimeLoader:
    ...
    def __post_init__(self) -> None:
        ...
        self._split_bounds_cache = {
            split: _split_bounds(total, split, self.split_ratio)
    ...
    #         index = min(self.cursor + offset, max(0, len(self.partition_data) - 1))
    #         batches.append(self.load_snapshot(index, split=split))
    #     self.cursor = min(len(self.partition_data), self.cursor + count)
    #     return batches

    # def _iter_range(self, split: str, count: int, chain: str | None = None) -> Iterable[DTDGBatch]:
    #     limit = min(len(self.partition_data), count)
    #     for offset in range(limit):
    #         batch = self.load_snapshot(offset, split=split)
    ...
    def iter_eval(self, split: str = "val") -> Iterable[DTDGBatch]:
        # Stateful eval: warm up RNN over train snapshots, then score val snapshots.
        # Mirrors FlareDTDG eval_epoch: iterate ALL snapshots, skip loss for train portion.
    ...
    def iter_predict(self, split: str = "test") -> Iterable[DTDGBatch]:
        # Stateful predict: same warmup logic as eval.
        _, train_end = self._split_bounds_cache["train"]
