...
def snapshot_to_partitiondata_tensors(
    ...
    Returns:
        Dict with keys: 'src_ids', 'dst_ids', 'edge_src', 'edge_dst', 'edge_ptr',
        'dst_chunk', 'edge_ids', 'edge_ts'
    """
    ...
    dst_ids, edge_dst_local = torch.unique_consecutive(sorted_dst, return_inverse=True)
    unique_src = torch.unique(sorted_src)
    src_is_local = torch.isin(unique_src, dst_ids)
    remote_src_ids = unique_src[~src_is_local]
    all_src_ids = torch.cat([dst_ids, remote_src_ids])

    map_size = int(node_to_chunk.numel()) if node_to_chunk is not None else int(torch.max(torch.cat([all_src_ids, sorted_dst])).item()) + 1
    src_map = torch.full((map_size,), -1, dtype=torch.long, device=sorted_src.device)
    src_map[all_src_ids] = torch.arange(all_src_ids.numel(), dtype=torch.long, device=sorted_src.device)
    edge_src_local = src_map[sorted_src]

    edge_counts = torch.bincount(edge_dst_local, minlength=int(dst_ids.numel()))
    edge_ptr = torch.empty(dst_ids.numel() + 1, dtype=torch.long, device=sorted_dst.device)
    edge_ptr[0] = 0
    edge_ptr[1:] = torch.cumsum(edge_counts, dim=0)
    edge_dst_ptr = torch.arange(dst_ids.numel() + 1, dtype=torch.long, device=sorted_dst.device)

    return {
        "src_ids": remote_src_ids,
        "dst_ids": dst_ids,
        "edge_src": edge_src_local,
        "edge_dst": edge_dst_ptr,
        "edge_ptr": edge_ptr,
...
def reconstruct_edge_index_from_snapshot(
    ...
    Args:
        ...
        edge_dst: Destination row indices aligned with edge_ptr. The last
            entry is a sentinel and is not a real destination row.
        edge_ptr: CSC pointer into edge_src/edge_dst, length num_dst_nodes + 1
    ...
    if edge_dst.numel() != edge_ptr.numel():
        raise ValueError("edge_dst must have the same length as edge_ptr")
    if edge_dst.numel() != dst_ids.numel() + 1:
        raise ValueError("edge_dst/edge_ptr length must equal dst_ids length + 1")
    expected_dst = torch.arange(edge_dst.numel(), dtype=edge_dst.dtype, device=edge_dst.device)
    if torch.any(edge_dst != expected_dst):
        raise ValueError("edge_dst is inconsistent with edge_ptr")
    edge_dst_from_ptr = torch.repeat_interleave(
        edge_dst[:-1].to(edge_ptr.device),
        edge_ptr[1:] - edge_ptr[:-1],
    )

    if not global_ids:
        return edge_src, edge_dst_from_ptr.to(edge_src.device)
    ...
    edge_dst_global = dst_ids[edge_dst_from_ptr.to(dst_ids.device)]
