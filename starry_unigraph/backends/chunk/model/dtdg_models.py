"""DTDG (discrete-time) temporal GNN models for the chunk backend.

Ported from backends/dtdg/runtime/models.py; STGraphBlob is now sourced
from the chunk runtime (stg_loader) instead of dtdg/runtime/state.

Built-in models:
- :class:`FlareEvolveGCN` — EvolveGCN-H: GRU-evolving GCN weight matrix.
- :class:`FlareTGCN`      — Temporal GCN: GCN + GRU cell per snapshot.
- :class:`FlareMPNNLSTM`  — MPNN-LSTM: GCN + two stacked LSTM cells.

All models accept either a single DGLGraph or a :class:`STGraphBlob`
(multi-frame sliding window).  Use :func:`build_flare_model` to instantiate
by name.
"""
...
from starry_unigraph.backends.chunk.runtime.stg_loader import STGraphBlob
        ...
        if graph.is_block:
            if x.size(0) == graph.num_src_nodes():
                ...
                if route is not None and route.send_index is not None and dist.is_available() and dist.is_initialized():
                    ...
                    src_x = torch.cat([x, extra], dim=0)
            else:
                raise ValueError(
                    f"Unexpected feature rows for block: got {x.size(0)}, "
...
class FlareEvolveGCN(nn.Module):
    ...
    Forward::
        ...
        # outputs: list[Tensor] if STGraphBlob, single Tensor if DGLGraph
...
class FlareTGCN(nn.Module):
    ...
    The GCN produces 3× hidden features split into (u, r, c) GRU gates.
...
class FlareMPNNLSTM(nn.Module):
    ...
    Uses ``flare_fetch_state`` / ``flare_store_state`` on each graph for
    per-snapshot state persistence (patched by RNNStateManager).
    The hidden state is a 4-tuple ``(h1, c1, h2, c2)``.
    ...
    Forward::
        ...
        outputs, h = model(blob_or_graph, state=None)
        # h: (h_layer1, c_layer1, h_layer2, c_layer2)
    """
...
class _FlareModelSpec:
    model_cls: type[nn.Module]


_MODEL_SPECS: dict[str, _FlareModelSpec] = {
    "evolvegcn": _FlareModelSpec(FlareEvolveGCN),
    "tgcn":      _FlareModelSpec(FlareTGCN),
    "mpnn_lstm": _FlareModelSpec(FlareMPNNLSTM),
    "gcn":       _FlareModelSpec(FlareTGCN),
}


def build_flare_model(
    model_name: str,
    input_size: int,
    hidden_size: int,
    output_size: int,
) -> nn.Module:
    """Instantiate a Flare DTDG model by name.

    Args:
        model_name: ``"evolvegcn"``, ``"tgcn"``, ``"mpnn_lstm"``, or ``"gcn"``.
        input_size: Input node feature dimension.
        hidden_size: GCN + recurrent hidden dimension.
    ...
    Example::
        ...
        model = build_flare_model("mpnn_lstm", input_size=32,
    ...
    if key not in _MODEL_SPECS:
        raise KeyError(
            f"Unknown flare model '{model_name}'. "
            f"Choose from: {list(_MODEL_SPECS)}"
        )
    return _MODEL_SPECS[key].model_cls(
...
def extract_graph_labels(blob_or_graph: STGraphBlob | DGLGraph) -> torch.Tensor | None:
    """Extract ground-truth labels from the last graph in a blob or single graph.

    Returns a ``[N, K]`` tensor, or ``None`` if no labels are present.
    """
