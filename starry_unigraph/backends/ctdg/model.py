...
def build_dgl_block(
    ...
    Stores global node IDs in srcdata['__ID'] (MemShare convention).
    Caller fills srcdata['h'] via memory[b.srcdata['__ID']] after this call.
    Returns None if no edges were sampled.
    """
...
class CTDGMemoryUpdater(nn.Module):
    """Updates per-node memory from K-slot mailbox history."""
