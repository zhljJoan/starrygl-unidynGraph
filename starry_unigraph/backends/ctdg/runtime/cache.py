"""Historical memory cache with adaptive sync threshold.

:class:`CTDGHistoricalCache` caches the last-synced memory for shared nodes
and uses cosine-distance change detection to skip redundant distributed
syncs.  :class:`AdaParameter` auto-tunes the threshold based on the ratio
of sync latency to compute latency.
"""
...
class CTDGHistoricalCache:
    ...
    def historical_check(
        ...
        """Check which shared nodes have drifted beyond the sync threshold.

        Computes cosine distance between current and cached memory and
        returns True for nodes that exceed ``(1 - alpha)``.

        Args:
            shared_node_local_indices: Local node IDs of shared nodes, ``[S]``.
            current_memory: Current memory vectors, ``[S, D]``.

        Returns:
            Boolean mask ``[S]``, True for nodes needing sync.
        """
