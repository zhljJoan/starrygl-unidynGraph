...
if TYPE_CHECKING:
    from .historical_cache import CTDGHistoricalCache
...
class CTDGMemoryBank:
    ...
    def gather(self, node_ids: torch.Tensor) -> torch.Tensor:
        """Return memory for global node_ids, placed on self.device."""
    ...
    def read_mailbox(self, node_ids: torch.Tensor) -> torch.Tensor:
        """Return K-slot mailbox [M, K, slot_width] on self.device."""
        loc = self._to_local(node_ids)        # CPU
        valid = loc >= 0
    ...
    def wait_pending_syncs(self) -> None:
        """Drain pending tensor all_to_all syncs."""
