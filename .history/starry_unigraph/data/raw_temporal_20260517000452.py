...
def _get_snapshots_slice(events: RawTemporalEvents, num_windows: int = 0, window_size: float=0, lags: int = 0, skip_time: int = 0) -> torch.Tensor:
    ...
    for i in range(num_windows - lags):
        mask = ((tss >= start_ts + i * time_itv) & (tss < start_ts + (i + lags + 1) * time_itv)).nonzero(as_tuple=False) if (tss >= start_ts + i * time_itv).any() else None
...
def build_snapshot_dataset_from_events(events: RawTemporalEvents, input_x = None, input_y = None,
    ...
    if slice_config['use_batch_split'] is True:
        ...
        windows_slices = windows_slices[:-1], windows_slices[1:]
    else:
        if windows_slices is None:
            if slice_config['batch_size'] is None:
    ...
    snaps = len(windows_slices[0])
    dataset: list[dict[str, Any]] = []
    in_degrees: list[torch.Tensor] = []
    for snap_idx in range(snaps):
        start= windows_slices[0][snap_idx]
        end = windows_slices[1][snap_idx]
        if start >= end:
