...
def _get_batch_snapshots(self, num_edges, batch_size):
    src, dst, wei, tss = self._get_uvwt(data)
    src_mapped, dst_mapped, num_nodes = self._get_mapped_uv(src, dst)
    ...
    windows_slices = torch.arange(num_edges,batch_size, dtype=torch.long)
    windows_slices = torch.stack((windows_slices, windows_slices[1:] + [num_edges]))
    return torch.arange(num_edges,batch_size, dtype=torch.long).tolist() + [num_edges]
