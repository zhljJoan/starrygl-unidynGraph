...
def _get_snapshots_slice(events: RawTemporalEvents, window_size: float, skip_time: int = 0) -> torch.Tensor:
    tss = events.ts 
    uniq_tss = torch.unique(tss)
    uniq_tss = np.sort(uniq_tss)[skip_time:]

def _get_batch_snapshots(events: RawTemporalEvents, num_edges:int,  batch_size: int, skip_time: int = 0    ) -> torch.Tensor:
    if batch_size <= 0:
        raise ValueError("batch_size must be positive")
    tss = events.ts
    uniq_tss = torch.unqiue(tss)
    if skip_time > 0 and skip_time < uniq_tss.size:
        start_pos = tss > unique_tss[skip_time]
    windows_slices = torch.arange(0, num_edges, batch_size, dtype=torch.long)
