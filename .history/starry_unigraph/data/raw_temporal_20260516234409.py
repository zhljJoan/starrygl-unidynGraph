...
def _get_snapshots_slice(events: RawTemporalEvents, num_windows: int, window_size: float, skip_time: int = 0) -> torch.Tensor:
    tss = events.ts 
    uniq_tss = torch.unique(tss)
    if skip_time > 0 and skip_time < uniq_tss.size(0):
        uniq_tss = uniq_tss[skip_time:]
    start_ts = uniq_tss[0]
    time_itv = (uniq_tss[-1] - uniq_tss[0]) / window_size
    if num_windows is None:
        num_windows = int((uniq_tss[-1] - uniq_tss[0]) / time_itv) + 1
