...
def build_snapshot_dataset_from_events(events: RawTemporalEvents, snaps: int, windows_slice = None, input_x = None, input_y = None) -> dict[str, Any]:
    ...
    for snap_idx in range(snaps):
        start = snap_idx * per_snap if input_range is 
        end = min(num_edges, start + per_snap)
        if start >= end:
