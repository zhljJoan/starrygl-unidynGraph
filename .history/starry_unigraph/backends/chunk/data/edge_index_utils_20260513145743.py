...
def snapshot_to_partitiondata_tensors(
    ...
    local_dst,local_ids = torch.unique_consecutive(sorted_dst,return_inverse=True)
    ...
    unique_src,ind = torch.unique(torch.cat([local_dst, sorted_src]), return_inverse=True)
    local_id_map = torch.zeros(ind.max()+1, dtype=torch.long, device=sorted_src.device) - 1
    local_id_map[ind[:local_dst.size(0)]] = local_ids
    local_id_map[local_id_map == -1] = torch.arange(local_dst.size(0), ind.max()+1, device=sorted_src.device)
    edge_src = local_id_map[ind[local_dst.size(0):]]
    edge_dst = torch.arange(local_dst.size(0), device=sorted_dst.device)[local_ids]
    src_ids = torch.cat([local_dst, unique_src[local_id_map > local_dst.size(0)]])
    #我目前的处理src_ids都是包含dst_ids的，后面需要对齐
    return {
        "src_ids": src_ids,
        "dst_ids": local_dst,
        "edge_src": edge_src,
