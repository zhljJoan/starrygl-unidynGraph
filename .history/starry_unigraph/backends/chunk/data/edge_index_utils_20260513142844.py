...
def get_csc_local_ids(
    edge_src: Tensor,
    edge_dst: Tensor,
    src_is_remote: Optional[Tensor] = None,
    ...
    unique_ids,ind = torch.unique(torch.stack((edge_dst,edge_src)).reshape(-1),return_inverse=True)
    idx = torch.arange(unique_ids.shape[0], dtype=torch.int32, device=edge_dst.device)
    idx[ind[unique_ids[:edge_dst.shape[0]]]]=torch.arange(edge_dst.shape[0], dtype=torch.int32, device=edge_dst.device)
    idx[ind[unique_ids[edge_dst.shape[0]:]]]=torch.arange(edge_src.shape[0], dtype=torch.int32, device=edge_dst.device)
