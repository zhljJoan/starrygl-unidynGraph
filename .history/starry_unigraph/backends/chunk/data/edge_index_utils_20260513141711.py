...
def sort_edges_by_dst_chunk(
    ...
    sort_key = dst_chunks * edge_dst.max(). + edge_dst
    sort_order = torch.argsort(dst_chunks, stable=True)
...
def partition_nodes_by_locality(
    edge_src: Tensor,
    edge_dst: Tensor,
    local_node_ids: Optional[Tensor] = None,
) -> Tuple[Tensor, Tensor]:
    """Partition edge endpoints into local and remote nodes.

    Assumptions:
    - Destination nodes (edge_dst) are local to this partition
    - Source nodes (edge_src) may be local or remote
    ...
    Returns:
        (src_ids, dst_ids) where:
        - src_ids includes both local and remote (boundary) nodes, starting with dst_ids
        - dst_ids contains only local destination nodes (unique)
    """
    ...
    if local_node_ids is None:
        ...
        # Remote src nodes are those in src but not in dst
        src_is_local = torch.isin(unique_src, unique_dst)
        local_src = unique_src[src_is_local]
        remote_src = unique_src[~src_is_local]

        # Combined src space: [local_dst | remote_src]
        # But we want all unique nodes that appear as destination first
        return torch.cat([unique_dst, remote_src]), unique_dst
    else:
        # Use provided local_node_ids
        unique_src = torch.unique(edge_src)

        # Local nodes are intersection of edges and local_node_ids
        local_src = unique_src[torch.isin(unique_src, local_node_ids)]
        remote_src = unique_src[~torch.isin(unique_src, local_node_ids)]

        # Return: [local_dst | remote_src]
        return torch.cat([unique_dst, remote_src]), unique_dst
...
def snapshot_to_partitiondata_tensors(
    ...
    # suffix in src_ids.
    all_src_ids, dst_ids = partition_nodes_by_locality(sorted_src, sorted_dst)
