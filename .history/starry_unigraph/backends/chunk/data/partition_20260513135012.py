...
class PartitionData:
            ...
            for idx in range(len(self)):
                ...
                if global_ids:
                    src_global, dst_global = reconstruct_edge_index_from_snapshot(
                        src_ids, dst_ids, edge_src, edge_dst, global_ids=True
                    )
    ...
    def edge_events(self, snapshot_indexs: Optional[int|slice] = None) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        """Get temporal events for CTDG sampler.
        ...
        start_idx = 0 if snapshot_indexs is None else snapshot_index_indexs.start if isinstance(snapshot_indexs, slice) else snapshot_indexs
        end_idx = len(self) if snapshot_index is None else (snapshot_index + 1)

        for idx in range(start_idx, end_idx):
            src, dst = self.to_edge_index(snapshot_index=idx, global_ids=True)
