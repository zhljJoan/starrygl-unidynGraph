...
Backend-specific (CTDG):

- **TemporalTransformerConv** — CTDG multi-head attention
- **CTDGMemoryUpdater** — CTDG GRU memory bank
- CTDG runtime model families are assembled in
  ``backends/ctdg/runtime/models.py``
- DTDG runtime model families are assembled in
  ``backends/dtdg/runtime/models.py``
...
.. code-block:: python
    ...
    class GCNStack(nn.Module):
        ...
        def layerwise(self, blob_or_graph) -> List[Tensor]:
            """Apply GCN to sequence of async graphs (STGraphBlob). 
...
The modules in ``runtime.modules`` are intended to be reused across backends
and custom model implementations. When building on them externally, treat the
documented constructor and forward signatures as the primary contract and
verify behavior against the current release you depend on.
